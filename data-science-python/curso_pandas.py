...
#%% Aula 9 - Identificando e removendo outliers
import pandas as pd
import matplotlib.pyplot as plt

dados = pd.read_csv(r'csv/aluguel_essencial.csv',sep=';') 
dados_valor = dados.Valor
dados_valor_new = dados[dados.Valor < 5000].Valor

fig = plt.figure()
plt.subplot(221)
dados_valor.plot(kind='hist')
plt.subplot(222)
dados_valor_new.plot(kind='hist')
plt.subplot(223)
dados_valor.plot(kind='box')
plt.subplot(224)
dados_valor_new.plot(kind='box')
plt.show()
...
dados = pd.read_csv(r'csv/aluguel_amostra.csv',sep=';') 
...
#%% Aula 9 - Identificando e removendo outliers (continuação)
import pandas as pd
import matplotlib.pyplot as plt
...
print(f'limite_superior: \n{limite_superior}\n')

#%% Aula 9 - Identificando e removendo outliers (continuação)
dados_new = pd.DataFrame()
for tipo in grupo_tipo.groups.keys():
    eh_tipo = dados.Tipo == tipo # quem é do mesmo tipo
    eh_dentro_limite = (dados.Valor >= limite_inferior[tipo]) & (dados.Valor <= limite_superior[tipo]) # valores que estão dentro dos limites
    selecao = eh_tipo & eh_dentro_limite # quem é do mesmo tipo e tem valo dentro dos limites
    dados_selecao = dados[selecao]
    dados_new = pd.concat([dados_new,dados_selecao])
dados_new.boxplot(['Valor'],by=['Tipo'])
dados_new.to_csv(r'csv/aluguel_essencial_sem_outliers.csv',sep=';',index=False)
...
g3_dados = dados.Valor.sample(100) # amostra de 100 valores aleatorrios
g3_dados.reset_index(drop=True,inplace=True)
g3.plot(g3_dados)
g3.set_title('Plot amostra 100 valores')

g4_dados = dados.groupby('Tipo').Valor
label = g4_dados.mean().index
valores = g4_dados.mean().values # acessando valores de uma series
g4.bar(label,valores)
g4.set_title('Valor medio por tipo')

area.savefig(r'csv/grafico.png',dpi = 300, bbox_inches = 'tight')
